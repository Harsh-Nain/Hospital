...
export default function ChatArea({ selectedUser, currentUser, onBack }) {
    ...
    return (
        <div className="flex flex-col h-[88vh] sm:h-screen bg-gray-50 w-full">
            ...
            <div className="sticky top-0 z-30 w-full border-b border-white/60 bg-white/80 backdrop-blur-2xl shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
                <div className="flex items-center justify-between px-4 py-3 sm:px-5">
                    <div className="flex min-w-0 items-center gap-3">
                        <button type="button" onClick={onBack} aria-label="Go back" className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-gray-50 text-gray-700 ring-1 ring-gray-200 transition-all duration-200 hover:bg-gray-100 active:scale-95 md:hidden">
                            <FiArrowLeft size={20} />
                        </button>

                        <div className="relative shrink-0">
                            <div className="rounded-full bg-linear-to-tr from-indigo-500 via-violet-500 to-fuchsia-500 p-0.5 shadow-sm">
                                <img src={selectedUser?.image || "/default-avatar.png"} alt={selectedUser?.fullName || "User"} loading="lazy" className="h-11 w-11 rounded-full object-cover bg-white" />
                            </div>
                            <span className={`absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white shadow-sm ${selectedUser?.online ? "bg-emerald-500" : "bg-gray-400"}`} />
                        </div>

                        <div className="min-w-0">
                            <div className="flex min-w-0 items-center gap-2">
                                <h2 title={selectedUser?.fullName || ""} className="max-w-40 truncate text-[15px] font-semibold text-gray-900 sm:max-w-xs">
                                    {selectedUser?.fullName || "Unknown User"}
                                </h2>
                                <span className={`hidden rounded-full px-2 py-0.5 text-[10px] font-medium ring-1 sm:inline-flex ${selectedUser?.online ? "bg-emerald-50 text-emerald-700 ring-emerald-200" : "bg-gray-50 text-gray-500 ring-gray-200"}`}>
                                    {selectedUser?.online ? "Online" : "Offline"}
                                </span>
                            </div>
                            <p className="truncate text-xs text-gray-500">{selectedUser?.online ? "Active now" : "Last seen recently"}</p>
                        </div>
                    </div>

                    <div className="flex items-center gap-2">
                        <div className="hidden sm:block"><CallSection selectedUser={selectedUser} currentUser={currentUser} /></div>
                        <button type="button" aria-label="More options" className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-gray-50 text-gray-700 ring-1 ring-gray-200 transition-all duration-200 hover:bg-gray-100 active:scale-95">
                        ...
                        return (
                            <div key={i} className={`flex ${m.isMe ? "justify-end" : "justify-start"} group relative px-2`} onClick={(e) => { e.stopPropagation(); setActiveMsg((prev) => (prev === i ? null : i)); }}>
                                ...
                                <div className={`flex flex-col ${m.isMe ? "items-end cursor-default" : "items-start"} max-w-[80%]`}>
                                    ...
                                    {m.message && (
                                        <div className={` px-4 py-2 rounded-2xl text-sm shadow-lg backdrop-blur-md transition-all duration-200 wrap-break-words ${m.isMe ? `${location ? "bg-[linear-gradient(135deg,#0ea5e9,#38bdf8,#7dd3fc)]" : "bg-[linear-gradient(135deg,#10b981,#34d399,#6ee7b7)]"} text-white rounded-br-none shadow-sm hover:scale-[1.02]` : ` bg-white text-gray-800 rounded-bl-none border border-gray-200 shadow-sm`}`}>
                                    ...
                                    <div className={`flex items-center gap-1 mt-1 px-1 text-[10px] text-gray-400 ${m.isMe ? "justify-end" : "justify-start"}`}>
                                        ...
                                        {(!m.pending && m.isMe) && (<span className={`text-${m.isSeen ? `${location ? "sky" : "emerald"}` : "gray"}-500 font-bold`}> ✓✓</span>)}
                                    </div>
                                </div>
                            </div>
                        );
            ...
            <div className="p-3 bg-white border-t border-black/10 flex gap-2 items-center sticky bottom-0">
                ...
                <button onClick={sendMessage} className={`${location ? "bg-sky-500 hover:bg-sky-600" : "bg-emerald-500 hover:bg-emerald-600"} text-white p-2 rounded-full`}><FiSend size={16} /></button>
            </div>
        </div>
    );
}
