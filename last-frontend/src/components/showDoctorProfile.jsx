    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl shadow-xl p-6">
        ...
        <div className="mb-6">
          ...
          {slots.length === 0 ? (<p className="text-gray-500 text-sm">No slots available</p>) : (
            <div className="flex flex-wrap gap-3">
              {slots.map((slot) => (
                <button key={slot.id} onClick={() => setSelectedSlot(slot.id)} className={`p-3 rounded-xl border text-sm transition min-w-27.5    ${(selectedSlot === slot.id && !isAdmin) ? "bg-sky-500 text-white border-sky-500" : "bg-sky-50 border-sky-200 hover:bg-sky-100"}`}>
        ...
        {!isAdmin && selectedSlot && (
          <button onClick={confirmAppointment} className="w-full bg-sky-500 text-white py-2 rounded-xl font-medium hover:shadow-lg transition">
            Confirm Appointment
          </button>
        )}
