...
export default function NextAppoitment({ id, setNextAppoitment, patientId, isAdmin, }) {
    ...
    return (
        <div className="fixed top-0 left-0 z-50 flex items-center w-full h-screen justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="relative w-full max-w-4xl mb-20 lg:mb-0 max-h-[92vh] overflow-hidden rounded-3xl bg-white/90 backdrop-blur-xl shadow-[0_20px_60px_rgba(0,0,0,0.15)]">
                ...
                <button onClick={() => setNextAppoitment(null)} className="absolute cursor-pointer top-4 right-4 z-50 w-10 h-10 flex items-center justify-center rounded-xl bg-white shadow-sm hover:bg-red-50 hover:scale-105 transition">
                    <RxCross1 size={18} />
                </button>
                        ...
                        <div className="flex gap-2 overflow-x-auto">
                            {["About", "Slots", "Reviews"].map((tab) => (
                                <button key={tab} onClick={() => { setActiveTab(tab); }} className={`relative flex-1 rounded-2xl px-5 py-3 text-sm font-semibold transition duration-300 ${activeTab === tab ? "bg-linear-to-r from-sky-500 via-sky-500 to-sky-500 text-white shadow-lg shadow-sky-200" : "text-gray-500 hover:bg-gray-100/80 hover:text-gray-800"}`}>
                                    {activeTab === tab && (<span className="absolute inset-x-6 -bottom-1 h-1 rounded-full bg-white/70"></span>)}{tab}
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    {activeTab === "About" && (
                        <div className="space-y-8 animate-in fade-in duration-500 sm:mb-7">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                                {[{ label: "Consultation Fee", value: `₹${doctor?.fee}`, icon: <FaMoneyBillWave className="text-emerald-500 text-xl" />, bg: "from-emerald-50 to-white" }, { label: "Experience", value: `${doctor?.experience} yrs`, icon: <FaBriefcaseMedical className="text-sky-500 text-xl" />, bg: "from-sky-50 to-white" }, { label: "Patients", value: reviews.length, icon: <FaUsers className="text-violet-500 text-xl" />, bg: "from-violet-50 to-white" }].map((item, i) => (
