    ...
    return (
        <>
            <div className="p-3 border-b border-black/10 flex items-center gap-3 bg-white sticky top-0 z-10 ">
                ...
                <img src={selectedUser.image} className="w-10 h-10 rounded-full object-cover" alt="" />
                    ...
                    messages.map((msg, i) => (
                        <div key={i} className={`flex ${msg.isMe ? "justify-end" : "justify-start"}`}>
                            <div className={`flex flex-col ${msg.isMe ? "items-end" : "items-start"} max-w-[80%]`}>
                                {msg.fileUrl && (<img src={msg.fileUrl} className={`rounded-xl ${msg.isMe ? "rounded-br-none" : "rounded-bl-none"} mb-1 max-h-60 w-full object-cover border border-black/5`} />)}
                                {msg.message && (<div className={`px-3 py-2 rounded-2xl text-sm shadow-sm word-break-word ${msg.isMe ? "bg-linear-to-r from-sky-400 to-blue-500 text-white " : "bg-black/5 text-gray-800"}`}>{msg.message}</div>)}
                                <div className={`flex items-center gap-1 mt-1 text-[10px] text-gray-400 ${msg.isMe ? "justify-end" : "justify-start"}`}>
                                    ...
                                    {!msg.pending && msg.isMe && (<span className={`${msg.isSeen ? "text-blue-500" : "text-gray-400"} font-bold`}>✓✓</span>)}
                                </div>
                            </div>
                        </div>
                    ))
